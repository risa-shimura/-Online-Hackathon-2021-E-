...
(async () => {
  ...
  const sock = new WebSocket( ws_scheme + "://" + window.location.host + "/ws/chat/" );
  ...
  sock.onmessage = (event) => {
    ...
    if (data.message.userId === 1) {
      user1.style.top = `${data.message.y}px`;
      user1.style.left = `${data.message.x}px`;
      user1.style.zIndex = `${data.message.zIndex}`;
      user1Name.textContent = data.message.name;
    }
    if (data.userId === 2) {
      user2.style.top = `${data.message.y}px`;
      user2.style.left = `${data.message.x}px`;
      user2.style.zIndex = `${data.message.zIndex}`;
      user2Name.textContent = data.message.name;
    }
    if (data.message.userId === 3) {
      user3.style.top = `${data.message.y}px`;
      user3.style.left = `${data.message.x}px`;
      user3.style.zIndex = `${data.message.zIndex}`;
      user3Name.textContent = data.message.name;
    }
    if (data.message.userId === 4) {
      user4.style.top = `${data.message.y}px`;
      user4.style.left = `${data.message.x}px`;
      user4.style.zIndex = `${data.message.zIndex}`;
      user4Name.textContent = data.message.name;
    }
    if (data.message.userId === 5) {
      user5.style.top = `${data.message.y}px`;
      user5.style.left = `${data.message.x}px`;
      user5.style.zIndex = `${data.message.zIndex}`;
      user5Name.textContent = data.message.name;
    }
    if (data.message.userId === 6) {
      user6.style.top = `${data.message.y}px`;
      user6.style.left = `${data.message.x}px`;
      user6.style.zIndex = `${data.message.zIndex}`;
      user6Name.textContent = data.message.name;
    }
    if (data.message.userId === 7) {
      user7.style.top = `${data.message.y}px`;
      user7.style.left = `${data.message.x}px`;
      user7.style.zIndex = `${data.message.zIndex}`;
      user7Name.textContent = data.message.name;
    }
    if (data.message.userId === 8) {
      user8.style.top = `${data.message.y}px`;
      user8.style.left = `${data.message.x}px`;
      user8.style.zIndex = `${data.message.zIndex}`;
      user8Name.textContent = data.message.name;
    }
    if (data.message.userId === 9) {
      user9.style.top = `${data.message.y}px`;
      user9.style.left = `${data.message.x}px`;
      user9.style.zIndex = `${data.message.zIndex}`;
      user9Name.textContent = data.message.name;
    }
    if (data.message.userId === 10) {
      user10.style.top = `${data.message.y}px`;
      user10.style.left = `${data.message.x}px`;
      user10.style.zIndex = `${data.message.zIndex}`;
      user10Name.textContent = data.message.name;
    }
  };
  ...
  const URL = "http://52.194.255.10:8080/shops/userinfo/"
  ...
  // 音楽関連
  // const noise = new Audio("../audio/noise.mp3""{% static 'noise.mp3' %}");
  // const music = new Audio("../audio/noise.mp3");
  // noise.loop = true;
  // music.loop = true;
  // noise.volume = 0.8;
  // music.volume = 0;
